...
const handler = createMcpHandler(
	async (server) => {
		...
		// Tool 2: Search transactions by action date (for finding truly new awards)
		server.tool(
			"search_transactions",
			...
			async ({
				...
				try {
					const filters: any = {
						award_type_codes: awardTypeCodes || ['A', 'B', 'C', 'D'],
					};

					// Action date is required for transactions
					const end = actionEndDate || actionStartDate;
					filters.time_period = [{
					...
					// Use the transaction search endpoint
					...
					const summary = `Found ${result.page_metadata?.total || 0} transactions with action dates ${actionStartDate} to ${end} (showing ${result.results?.length || 0})`;
		...
		// Tool 3: Get detailed award information
		server.tool(
			"get_award_details",
